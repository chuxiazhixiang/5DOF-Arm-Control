...
class MovewithFace(object):
    ...
    def generate_fuzzy_control(self):
        ...
        theta_x['L']=fuzz.trapmf(theta_x.universe,[-170,-170,-85,0])  # Left
        theta_x['S']=fuzz.trimf(theta_x.universe,[-85,0,85])         # Stop
        theta_x['R']=fuzz.trapmf(theta_x.universe,[0,85,170,170])     # Right

        theta_y['D']=fuzz.trapmf(theta_y.universe,[-110,-110,-55,0])  # Down
        theta_y['S']=fuzz.trimf(theta_y.universe,[-55,0,55])         # Stop
        theta_y['U']=fuzz.trapmf(theta_y.universe,[0,55,110,110])     # Up

        rule1=ctrl.Rule(vx['P'],theta_x['L'])  # 如果vx是Positive，theta_x向右
        rule2=ctrl.Rule(vx['N'],theta_x['R'])  # 如果vx是Negative，theta_x向左
        rule3=ctrl.Rule(vx['Z'],theta_x['S'])  # 如果vx是Zero，theta_x停止

        rule4=ctrl.Rule(vy['P'],theta_y['D'])  # 如果vy是Positive，theta_y向上
        rule5=ctrl.Rule(vy['N'],theta_y['U'])  # 如果vy是Negative，theta_y向下
